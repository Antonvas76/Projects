random
def play():
    user = input("What's your choice? 'r' for rock, 'p' for paper, 's' for scissors/n")
    user = user.lower()
...
def is_win(player, opponent):
    ...
    if (player == 'r' and opponent == 's' or (player == 's' and opponent == 'p') or (player == 'p' and opponent)
        return True
         return False )
...
def play_best_of (n):
    ...
    while player_wins < wins_necessary and computer_wins < wins_necessary:
        ...
        #tie
        if result == 0:
            print('It is a tie. You and the computer have both chosen {}. /n'.format(user))
            # you win
        elif result ==1:
            player_wins += 1
            print('You chose {} and the computer chose {}. You won!/n' .format(user, computer))
        else: 
            computer_wins += 1
            print('You chose {} and the computer chose {}. You lost : (/n'.format(user, computer))
            print('/n')
            
            if player_wins > computer_wins:
                print('You have won the best of {} games! What a champ :D' .format(n))
            else:
                print('Unfortunetly, the computer has won the best of {} games. Better luck next time')
